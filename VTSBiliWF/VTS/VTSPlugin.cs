...
namespace VTS {
    ...
    public abstract class VTSPlugin
    {
        ...
        public void Initialize(VTSWebSocket vtsWebSocket, IWebSocket webSocket, IJsonUtility jsonUtility, ITokenStorage tokenStorage, Action onConnect, Action onDisconnect, Action onError){
            this._tokenStorage = tokenStorage;
            this._socket = vtsWebSocket;
            this._socket.Initialize(webSocket, jsonUtility);
            this._socket.Connect(() => {
                // If API enabled, authenticate
                Authenticate(
                    (r) => {
                        Debug.WriteLine(new JsonUtilityImpl().ToJson(r));
        ...
        private void Reauthenticate(Action onConnect, Action onError){
            Debug.WriteLine("Token expired, acquiring new token...");
            this._isAuthenticated = false;
        ...
        public void TintArtMesh(ColorTint tint, float mixWithSceneLightingColor,  ArtMeshMatcher matcher, Action<VTSColorTintData> onSuccess, Action<VTSErrorData> onError){
            ...
            _socket.Send(request, onSuccess, onError);
        }
        ...
        private string EncodeIcon(Image icon){
            try{
                if(icon == null || icon.Width != 128 || icon.Height != 128){
                    Debug.WriteLine("Icon resolution must be exactly 128*128 pixels!");
                    return null;
                }
                return Convert.ToBase64String(converterDemo(icon));
            }catch(Exception e){
                Debug.WriteLine(e);
            }
            return null;
