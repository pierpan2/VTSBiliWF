    ...
    public class WebSocketImpl : IWebSocket
    {
        ...
        public async void Start(string URL, System.Action onConnect, System.Action onDisconnect, System.Action onError)
        {
            try{
                ...
                System.Diagnostics.Debug.WriteLine("Connecting to: " + serverUri);
                await this._ws.ConnectAsync(serverUri, token);
                while(IsConnecting())
                {
                    System.Diagnostics.Debug.WriteLine("Waiting to connect...");
                    await Task.Delay(10);
                }
                System.Diagnostics.Debug.WriteLine("Connect status: " + this._ws.State);
        ...
        private void Reconnect(){
            ...
            Start(this._url, this._onReconnect, this._onDisconnect, async () => { 
                // keep retrying 
                System.Diagnostics.Debug.WriteLine("Reconnect failed, trying again!");
                await Task.Delay(2);
                Reconnect();
            } );
        }
        ...
        private void Dispose(){
            System.Diagnostics.Debug.WriteLine("Disposing of socket...");
        ...
        public void Send(string message)
        {
            ...
            // System.Diagnostics.Debug.WriteLine("Message to queue for send: " + buffer.Length + ", message: " + message);
        ...
        {
            System.Diagnostics.Debug.WriteLine("WebSocket Message Sender looping.");
                ...
                if(!this._sendQueue.IsEmpty && this.IsConnectionOpen() && _sendQueue.TryDequeue(out msg))
                {
                    try{
                        ...
                        || e is System.Net.Sockets.SocketException){
                            System.Diagnostics.Debug.WriteLine("Socket exception occured, reconnecting...");
                            Reconnect();
                        }
                    }
                }
        ...
        {
            System.Diagnostics.Debug.WriteLine("WebSocket Message Receiver looping.");
            string result;
            while(!token.IsCancellationRequested)
            {
